        ...
        for(int t=0; t<T; t++) {
            ...
            if(t==0) {
                ...
                for(int n=0; n<N; n++) {
                    for(int nt=0; nt<Nt; nt++) {
                        // Keep inactive
                        if(gsl_rng_uniform(semilla[omp_get_thread_num()])<am[nt]) {
                            setint_Xt(0,Xt,nt,n,t,Nt,N,T);
                        // Activate
                        } else {
                            if(lHead==0) {
                                setint_Xt(1+(int)((Q-1.0)*gsl_rng_uniform(semilla[omp_get_thread_num()])),Xt,nt,n,t,Nt,N,T);
                            } else {
                ...
                for(int n=0; n<N; n++) {
                    ...
                    for(int nt=0; nt<Nt; nt++) {
                        ...
                        // If it was active
                            ...
                            // Deactivate
                            if(gsl_rng_uniform(semilla[omp_get_thread_num()])<bm[nt]) {
            ...
            if(flagComplex) {
                ...
                for(int n=0; n<N; n++) {
                    ...
                    while((r<L)&&(t-r>=0)) {
                        for(int nr=0; nr<Nr; nr++) {
                            for(int nt=0; nt<Nt; nt++) {
                                sumr[N*nr+n] += getdouble_3D(Hr,nr,nt,r,Nr,Nt,L)*Cr[getint_forC_Xt(Xt,nt,nCur,t-r,Nt,N,T,head,offState)] \
                                                -getdouble_3D(Hi,nr,nt,r,Nr,Nt,L)*Ci[getint_forC_Xt(Xt,nt,nCur,t-r,Nt,N,T,head,offState)];
                                sumi[N*nr+n] += getdouble_3D(Hr,nr,nt,r,Nr,Nt,L)*Ci[getint_forC_Xt(Xt,nt,nCur,t-r,Nt,N,T,head,offState)] \
                    ...
                    logW[t][n] = 0;
                    for(int nr=0; nr<Nr; nr++) {
                        aux1 = getdouble_2D(Yr,nr,t,Nr,T)-sumr[N*nr+n];
                        aux2 = getdouble_2D(Yi,nr,t,Nr,T)-sumi[N*nr+n];
    ...
    if(flagComplex) {
        for(int tau=t; tau<=maxtau; tau++) {
            ...
            for(int r=maxr; r>=minr; r--) {
                ...
                computeFactor1_parallelSubRoutine1(Nt,Nr,N,L,T,indaux,r,maxr,tau,ancContribr,ancContribi,Cr,Ci,Yr,Yi,a_ind,Act_ancB,Xt,Hr,Hi,head,offState);
            }
            ...
            for(int n=0; n<N; n++) {
                ...
                double aux1;
                double aux2;
                for(int nrS=0; nrS<Nr; nrS++) {
                    aux1 = Ypredr[nrS] - getdouble_2D(ancContribr[L-2],nrS,n,Nr,N);
                    aux2 = Ypredi[nrS] - getdouble_2D(ancContribi[L-2],nrS,n,Nr,N);
        ...
        for(int tau=t; tau<=maxtau; tau++) {
            ...
            for(int r=maxr; r>=minr; r--) {
                int indaux = tau-t+L-r-1;
                computeFactor1_parallelSubRoutine2(Nt,Nr,N,L,T,indaux,r,maxr,tau,ancContribr,ancContribi,Cr,Ci,Yr,Yi,a_ind,Act_ancB,Xt,Hr,Hi,head,offState);
            }
...
void computeFactor1_parallelSubRoutine1(int Nt, int Nr, int N, int L, int T, int indaux, int r, int maxr, int tau, double **ancContribr, double **ancContribi, double *Cr, double *Ci, double *Yr, double *Yi, int *a_ind, bool *Act_ancB, int16_T *Xt, double *Hr, double *Hi, double *head, int offState) {
    #pragma omp parallel for schedule(dynamic,10)
    for(int n=0; n<N; n++) {
        if(getbool_2D(Act_ancB,n,indaux,N,L-1)) {
            for(int nr=0; nr<Nr; nr++) {
                double valr = 0;
                double vali = 0;
                for(int nt=0; nt<Nt; nt++) {
...
void computeFactor1_parallelSubRoutine2(int Nt, int Nr, int N, int L, int T, int indaux, int r, int maxr, int tau, double **ancContribr, double **ancContribi, double *Cr, double *Ci, double *Yr, double *Yi, int *a_ind, bool *Act_ancB, int16_T *Xt, double *Hr, double *Hi, double *head, int offState) {
    #pragma omp parallel for schedule(dynamic,10)
    for(int n=0; n<N; n++) {
        if(getbool_2D(Act_ancB,n,indaux,N,L-1)) {
            for(int nr=0; nr<Nr; nr++) {
