function [Sest SeqEst nest] = sample_Z_FFBS_laplace(data,samples,hyper,param)
...
for it=1:param.ffbs.Niter
    for m=1:Mest
        ...
        t = 1;
        s2P = s2y/sum(W(m,:).^2);
        muP1 = s2P*(W(m,:)*Xhat(:,t)/s2y+1/bX);
        muP2 = s2P*(W(m,:)*Xhat(:,t)/s2y-1/bX);
        fw(1,t) = -1/(2*s2y)*sum(Xhat(:,t).^2)...
                  +log(A(1,1));
        fw(2,t) = -log(2*bX)-1/(2*s2y)*sum(Xhat(:,t).^2)+0.5*log(2*pi*s2P)+muP1^2/(2*s2P)+log(normcdf(0,muP1,sqrt(s2P))+exp((muP2^2-muP1^2)/(2*s2P))*(1-normcdf(0,muP2,sqrt(s2P))))...
                  +log(A(1,2));
        ...
        SeqEst(m,t) = aux;
        if(aux~=0)
            I2 = exp((muP2^2-muP1^2)/(2*s2P))*(1-normcdf(0,muP2,sqrt(s2P)));
            I1 = normcdf(0,muP1,sqrt(s2P));
            p2 = I2/(I1+I2);
            flagPositive = (rand(1)<p2);
            if(flagPositive)
                Sest(m,t) = rmvnrnd(muP2,s2P,1,-1,0);
            else
                Sest(m,t) = rmvnrnd(muP1,s2P,1,1,0);
            end
        else
            Sest(m,t) = 0;
        end
